  ...
  async index(request, response) {
    ...
    if(name) {
      ...
      if(dishesByName.length == 0) {
        const dishesByIngredients = await knex("dish_ingredients").select("dishes.*").whereLike('dish_ingredients.name',`%${name}%`)
        .innerJoin('dishes', 'dishes.id', 'dish_ingredients.dish_id').orderBy('dishes.name').groupBy('dishes.id');
